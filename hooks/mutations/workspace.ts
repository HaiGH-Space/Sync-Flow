...
export const useCreateWorkspace = () => {
  ...
  return useMutation({
    mutationFn: (payload: CreateWorkspace) =>
...
export const useDeleteWorkspace = () => {
  ...
  return useMutation({
    mutationFn: (payload: { workspaceId: string }) =>
      workspaceService.deleteWorkspace(payload),
...
export const useAcceptWorkspace = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (payload: { token: string }) =>
      workspaceService.acceptWorkspace(payload),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: workspaceKeys.list() });
    },
  });
